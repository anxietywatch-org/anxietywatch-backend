...
user-secrets, never source control).

| Key | Purpose | Example env var |
| --- | --- | --- |
| `Ml:Inference:BaseUrl` | Absolute base URL of the ML service | `Ml__Inference__BaseUrl` |
...
Response (parsed into `MlInferenceResponse`):
...
- `UNEXPECTED` (other statuses, malformed success payload)
